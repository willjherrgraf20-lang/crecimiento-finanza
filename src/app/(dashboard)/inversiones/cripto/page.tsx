...
const defaultForm = (): FormState => ({ ticker: "", name: "", quantity: "", avgCostBasis: "", currency: "USDT" });
...
export default function CriptoPage() {
  ...
  async function fetchHoldings() {
    try {
      const res = await fetch("/api/inversiones/holdings?type=CRYPTO");
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-bold text-[var(--color-text-primary)]">Criptomonedas</h1>
          <p className="mt-0.5 text-sm text-[var(--color-text-secondary)]">Precios en tiempo real via Binance</p>
        </div>
      ...
      {showForm && (
        <form onSubmit={handleSubmit} className="rounded-xl border border-[var(--color-border)] bg-[var(--color-bg-surface)] p-5 space-y-4">
          <h2 className="text-sm font-semibold text-[var(--color-text-primary)]">Nueva criptomoneda</h2>
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              ...
              <input type="number" step="any" value={form.avgCostBasis} onChange={(e) => setForm((f) => ({ ...f, avgCostBasis: e.target.value }))}
                placeholder="45000"
                className="w-full rounded-lg border border-[var(--color-border)] bg-[var(--color-bg-elevated)] px-3 py-2 text-sm text-[var(--color-text-primary)] focus:outline-none focus:ring-1 focus:ring-[var(--color-accent)]" />
            </div>
            <div>
      ...
      {loading ? (
        ...
        <div className="flex h-40 items-center justify-center rounded-xl border border-[var(--color-border)] bg-[var(--color-bg-surface)]">
          <p className="text-sm text-[var(--color-text-secondary)]">Sin criptos. Agrega tu primer holding.</p>
        </div>
      ) : (
